...
## 当前里程碑：1.4 专注关联与时间线归组

当前迭代分支为 `codex/1.4.0-focus-timeline-task-edit`，版本 `1.4.0`。本轮让所有任务新建表单可取消，修复专注切换后的想法关联，按两分钟阈值归组时间线，并允许一级任务调整重要性与紧急性；详细行为见 [1.4 说明](docs/V1_4_FOCUS_TIMELINE_TASK_EDIT.md)。
...
- 时间线归组：相邻事件不足两分钟时共用一个时间节点，但 SQLite 原始事件仍逐条保留。

- React + TypeScript + Vite 界面；“我的一天”始终作为主页面，随笔、Obsidian 与设置使用弹窗。
- 父子任务均可修改内容与当天时间段；一级任务还可修改重要性与紧急性；四象限支持独立滚动、快捷新增和完成项置底。
